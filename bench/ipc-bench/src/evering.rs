...
use evering::{
    msg::{Envelope, MoveMsg},
    os::{
        FdBackend,
        unix::{AddrSpec, UnixFd},
    },
    perlude::talc::{
        Access, MapBuilder, Session,
        channel::{
            CachePool, Completer, QueueChannel, ReqId, Submitter, TryRecvError, TrySendError,
            TrySubmitError,
        },
    },
};
        ...
        let server = s.spawn(|| {
            runtime.block_on(async move {
                ...
                loop {
                    ...
                    let req = MoveMsg::<[u8]>::detoken(req, &alloc).expect("should detoken");
                    check_req(bufsize, &req);
                    drop(req);

                    let (resp, _alloc) = MoveMsg::copy_from_slice(&respdata, &alloc);
                    ...
                    .map(|_| {
                        ...
                        tokio::spawn(async move {
                            for _ in 0..(iters / CONCURRENCY) {
                                let (req, alloc) = MoveMsg::copy_from_slice(&req_data, &alloc);
                                ...
                                let resp = MoveMsg::detoken(resp, alloc).expect("should detoken");
